...
Instead of interacting with snakemake rules directly, the main way to run Tourmaline V2 is through the `tourmaline.sh` script. This script allows you to run one or more of the workflow steps at a time, specify specific config files, and set the maximum number of cores. You must be located in the tourmaline directory when running it, however you can set the output file destinatUseage:  

```
conda activate snakemake
./tourmaline.sh --step [sample,repseqs,taxonomy] --configfile [config1,config2,config3] --cores N
```
...
Tourmaline V2 will currently work with an existing Snakemake and qiime2-2023.5 environments.

Tourmaline provides Snakemake rules for DADA2 (single-end and paired-end). It is run in 3 steps, each of which can be run alone or in succession via a shell script `tourmaline.sh`. Externally generated input files can be provided at each step, for example if you already have ASVs and just want to assign taxonomy. 

#### Steps

**Sample step**. Imports FASTQ data into a .qza format, optionally trims fastq files, provides general sequencing quality and trimming stats.  
...
**Repseqs step.** Runs denoising, generating a feature table (with read counts per sample) and representative sequences. Gives stats on ASV generation and features.  
...
Instead of one config.yaml file, Tourmaline V2 now has separate config files for each of the 3 steps. The configuration files can be named whatever you want, and don't need to be over written to 
...
Download reference database sequence and taxonomy files into the `00-data` folder. 

```
cd tourmaline/00-data
...
Edit FASTQ manifests `manifest_se.csv` and `manifest_pe.csv` in `00-data` so file paths match the location of your tourmaline directory. In the command below, replace `/path/to` with the location of your tourmaline directory. NOTE: you can now also run tourmaline without a manifest file, but just providing the directory for the sequence files.
