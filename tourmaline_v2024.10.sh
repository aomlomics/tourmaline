...
for index in "${!step_array[@]}"; do
    ...
    case $step in
        sample)
            ...
            if [[ "${paired}" = true ]]; then
                ...
                #snakemake --use-conda -s sample_step_paired.Snakefile --configfile $CONFIG --cores $CORES  --latency-wait 15
                    snakemake --use-conda -s sample_step_paired_v2024-10.Snakefile trim_pe_all --configfile $CONFIG --cores $cores  --latency-wait 15
                else
                    snakemake --use-conda -s sample_step_paired_v2024-10.Snakefile no_trim_pe_all --configfile $CONFIG --cores $cores --latency-wait 15
                fi;
            else
                #snakemake --use-conda -s sample_step_single.Snakefile --configfile $CONFIG --cores $CORES --latency-wait 15
                if [[ "${trim}" = true ]]; then
                    snakemake --use-conda -s sample_step_single_v2024-10.Snakefile trim_se_all --configfile $CONFIG --cores $cores  --latency-wait 15
                else
                    snakemake --use-conda -s sample_step_single_v2024-10.Snakefile no_trim_se_all --configfile $CONFIG --cores $cores  --latency-wait 15
                fi;
            fi;
            ;;
        repseqs)
            ...
            if [[ ${asv_method} = dada2pe ]]; then
                snakemake --use-conda -s repseqs_step_v2024-10.Snakefile run_dada2_pe_denoise --configfile $CONFIG --cores $cores  --latency-wait 15
            elif [[ "${asv_method}" = "dada2se" ]]; then
                snakemake --use-conda -s repseqs_step_v2024-10.Snakefile run_dada2_se_denoise --configfile $CONFIG --cores $cores  --latency-wait 15
            elif [[ "${asv_method}" = "deblur" ]]; then
                snakemake --use-conda -s repseqs_step_v2024-10.Snakefile run_deblur_se_denoise --configfile $CONFIG --cores $cores  --latency-wait 15
            else
        ...
        taxonomy)
            ...
            snakemake --use-conda -s taxonomy_step_v2024-10.Snakefile --configfile $CONFIG --cores $cores  --latency-wait 15
            ;;
        *)
