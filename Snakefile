...
rule tabulate_repseqs_to_filter:
    ...
    threads: config["other_threads"]
    shell:
        "cat {input.proptsv} | egrep -i 'outlier|true' | cut -f1,4 > {output.outliers}; "
        "cat {input.proptsv} | egrep -i 'taxonomy|unassigned' | cut -f1,5 > {output.unassigned}"
...
rule generate_report_md:
    ...
    threads: config["other_threads"]
    shell:
        r"echo '# Tourmaline Report' > {output};"
        r"echo '' >> {output};"
        r"echo 'View this HTML report with [Chrome](https://www.google.com/chrome/){{target=\"_blank\"}} or [Firefox](https://www.mozilla.org/en-US/firefox/new/){{target=\"_blank\"}} for best results.' >> {output};"
        r"echo '' >> {output};"
        r"echo 'To view the linked files below: ' >> {output};"
        r"echo '' >> {output};"
        r"echo '* QZV (QIIME 2 visualization): click to download, then drag and drop in [https://view.qiime2.org](https://view.qiime2.org){{target=\"_blank\"}}.' >> {output};"
        r"echo '* TSV (tab-separated values): click to download, then open in Microsoft Excel or Tabview (command line tool).' >> {output};"
        r"echo '* PDF (portable document format): click to open and view in new tab.' >> {output};"
        r"echo '* Markdown and text: click to open and view in new tab.' >> {output};"
        r"echo '' >> {output};"
        r"echo 'Note: Downloaded files can be deleted after viewing, as they are already stored in your Tourmaline directory.' >> {output};"
        r"echo '' >> {output};"
        r"echo 'For information on Tourmaline outputs, visit [https://github.com/aomlomics/tourmaline](https://github.com/aomlomics/tourmaline){{target=\"_blank\"}}.' >> {output};"
        r"echo '' >> {output};"
        r"echo '---' >> {output};"
        r"echo '' >> {output};"
        r"echo '## Metadata Summary' >> {output};"
        r"echo '' >> {output};"
        r"echo Markdown: [{input.mdsummary}]\(../{input.mdsummary}\){{target=\"_blank\"}} >> {output};"
        r"echo '' >> {output};"
        r"cat {input.mdsummary} >> {output};"
        r"echo '' >> {output};"
        r"echo '' >> {output};"
        r"echo '---' >> {output};"
        r"echo '' >> {output};"
        r"echo '## Fastq Sequences Information' >> {output};"
        r"echo '' >> {output};"
        r"echo '### Fastq Sequences per Sample' >> {output};"
        r"echo '' >> {output};"
        r"echo Markdown: [{input.fastq}]\(../{input.fastq}\){{target=\"_blank\"}} >> {output};"
        r"echo '' >> {output};"
        r"cat {input.fastq} >> {output};"
        r"echo '' >> {output};"
        r"echo '### Visualization of Fastq Sequences' >> {output};"
        r"echo '' >> {output};"
        r"echo QZV: [{input.visfastq}]\(../{input.visfastq}\){{target=\"_blank\"}} >> {output};"
        r"echo '' >> {output};"
        r"echo '---' >> {output};"
        r"echo '' >> {output};"
        r"echo '## Representative Sequences Information' >> {output};"
        r"echo '' >> {output};"
        r"echo '### Representative Sequences Properties Table' >> {output};"
        r"echo '' >> {output};"
        r"echo TSV: [{input.repseqstsv}]\(../{input.repseqstsv}\){{target=\"_blank\"}} >> {output};"
        r"echo '' >> {output};"
        r"echo 'Columns:' >> {output};"
        r"echo '' >> {output};"
        r"echo '* featureid' >> {output};"
        r"echo '* length - length (bp) not including gaps' >> {output};"
        r"echo '* gaps - gaps (bp) in multiple sequence alignment' >> {output};"
        r"echo '* outlier - outlier (True/False) determined by OD-seq' >> {output};"
        r"echo '* taxonomy - domain level' >> {output};"
        r"echo '* observations - total observations summed across all samples (unrarefied)' >> {output};"
        r"echo '* log10(observations) - log base-10 of total observations' >> {output};"
        r"echo '' >> {output};"
        r"echo '### Representative Sequences Properties Summary' >> {output};"
        r"echo '' >> {output};"
        r"echo Markdown: [{input.repseqsdescribe}]\(../{input.repseqsdescribe}\){{target=\"_blank\"}} >> {output};"
        r"echo '' >> {output};"
        r"cat {input.repseqsdescribe} >> {output};"
        r"echo '' >> {output};"
        r"echo '### Representative Sequences Properties Plot' >> {output};"
        r"echo '' >> {output};"
        r"echo PDF: [{input.repseqspdf}]\(../{input.repseqspdf}\){{target=\"_blank\"}} >> {output};"
        r"echo '' >> {output};"
        #"echo '<img src=\"../{input.repseqspdf}\" alt=\"../{input.repseqspdf}\" width=\"70%\"/>' >> {output};"
        r"echo '' >> {output};"
        r"echo 'Plot elements:' >> {output};"
        r"echo '' >> {output};"
        r"echo '* x: length (bp) not including gaps' >> {output};"
        r"echo '* y: gaps (bp) in multiple sequence alignment' >> {output};"
        r"echo '* color: taxonomy (domain)' >> {output};"
        r"echo '* size: log10(observations)' >> {output};"
        r"echo '* facets: outlier (True/False) determined by OD-seq' >> {output};"
        r"echo '' >> {output};"
        r"echo '### Representative Sequences Rooted Tree' >> {output};"
        r"echo '' >> {output};"
        r"echo QZV: [{input.repseqstree}]\(../{input.repseqstree}\){{target=\"_blank\"}} >> {output};"
        r"echo '' >> {output};"
        r"echo '### Representative Sequences to Filter' >> {output};"
        r"echo '' >> {output};"
        r"echo 'To filter, copy 02-output-{{method}}-{{filter}}/02-alignment-tree/repseqs_to_filter_outliers.tsv to 00-data/repseqs_to_filter_{{method}}.tsv, then run Snakemake in filtered mode. A file with unassigned repseqs is provided for reference; these sequences can be filtered by adding the term \"unassigned\" to \"exclude_terms\".' >> {output};"
        r"echo '' >> {output};"
        r"echo Outliers TSV: [{input.repseqsoutliers}]\(../{input.repseqsoutliers}\){{target=\"_blank\"}} >> {output};"
        r"echo '' >> {output};"
        r"echo Unassigned TSV: [{input.repseqsunassigned}]\(../{input.repseqsunassigned}\){{target=\"_blank\"}} >> {output};"
        r"echo '' >> {output};"
        r"echo '### Visualization of Representative Sequences' >> {output};"
        r"echo '' >> {output};"
        r"echo QZV: [{input.visrepseqs}]\(../{input.visrepseqs}\){{target=\"_blank\"}} >> {output};"
        r"echo '' >> {output};"
        r"echo '### Taxonomy Table' >> {output};"
        r"echo '' >> {output};"
        r"echo TSV: [{input.tsvtaxonomy}]\(../{input.tsvtaxonomy}\){{target=\"_blank\"}} >> {output};"
        r"echo '' >> {output};"
        r"echo '### Visualization of Taxonomy' >> {output};"
        r"echo '' >> {output};"
        r"echo QZV: [{input.vistaxonomy}]\(../{input.vistaxonomy}\){{target=\"_blank\"}} >> {output};"
        r"echo '' >> {output};"
        r"echo '### Taxonomy Reference Database' >> {output};"
        r"echo '' >> {output};"
        r"echo '{params.refdatabase}' >> {output};"
        r"echo '' >> {output};"
        r"echo '### Predicted Amplicon Type' >> {output};"
        r"echo '' >> {output};"
        r"echo Text: [{input.amplicontype}]\(../{input.amplicontype}\){{target=\"_blank\"}} >> {output};"
        r"echo '' >> {output};"
        r"echo '```' >> {output};"
        r"head -n 5 {input.amplicontype} >> {output};"
        r"echo '```' >> {output};"
        r"echo '' >> {output};"
        r"echo '---' >> {output};"
        r"echo '' >> {output};"
        r"echo '## Observation Table Information' >> {output};"
        r"echo '' >> {output};"
        r"echo '### Table Summary: Samples' >> {output};"
        r"echo '' >> {output};"
        r"echo Text: [{input.samples}]\(../{input.samples}\){{target=\"_blank\"}} >> {output};"
        r"echo '' >> {output};"
        r"echo '```' >> {output};"
        r"head -n 13 {input.samples} >> {output};"
        r"echo '```' >> {output};"
        r"echo '' >> {output};"
        r"echo '### Table Summary: Features' >> {output};"
        r"echo '' >> {output};"
        r"echo Text: [{input.features}]\(../{input.features}\){{target=\"_blank\"}} >> {output};"
        r"echo '' >> {output};"
        r"echo '```' >> {output};"
        r"head -n 13 {input.features} >> {output};"
        r"echo '```' >> {output};"
        r"echo '' >> {output};"
        r"echo '### Visualization of Table Summary' >> {output};"
        r"echo '' >> {output};"
        r"echo QZV: [{input.vistable}]\(../{input.vistable}\){{target=\"_blank\"}} >> {output};"
        r"echo '' >> {output};"
        r"echo '---' >> {output};"
        r"echo '' >> {output};"
        r"echo '## Taxonomic Diversity Results' >> {output};"
        r"echo '' >> {output};"
        r"echo '### Taxonomy Barplot' >> {output};"
        r"echo '' >> {output};"
        r"echo QZV: [{input.vistaxbar}]\(../{input.vistaxbar}\){{target=\"_blank\"}} >> {output};"
        r"echo '' >> {output};"
        r"echo '---' >> {output};"
        r"echo '' >> {output};"
        r"echo '## Alpha Diversity Results' >> {output};"
        r"echo '' >> {output};"
        r"echo '### Alpha Rarefaction' >> {output};"
        r"echo '' >> {output};"
        ...
        r"echo '' >> {output};"
        r"echo Shannon QZV: [{input.visshannongs}]\(../{input.visshannongs}\){{target=\"_blank\"}} >> {output};"
        r"echo '' >> {output};"
        r"echo '---' >> {output};"
        r"echo '' >> {output};"
        r"echo '## Beta Diversity Results' >> {output};"
        r"echo '' >> {output};"
        r"echo '### PCoA Emperor Plots' >> {output};"
        r"echo '' >> {output};"
        r"echo Bray-Curtis QZV: [{input.visbcemp}]\(../{input.visbcemp}\){{target=\"_blank\"}} >> {output};"
        r"echo '' >> {output};"
        r"echo Jaccard QZV: [{input.visjacemp}]\(../{input.visjacemp}\){{target=\"_blank\"}} >> {output};"
        r"echo '' >> {output};"
        r"echo Weighted UniFrac QZV: [{input.viswuemp}]\(../{input.viswuemp}\){{target=\"_blank\"}} >> {output};"
        r"echo '' >> {output};"
        r"echo Unweighted UniFrac QZV: [{input.visuwuemp}]\(../{input.visuwuemp}\){{target=\"_blank\"}} >> {output};"
        r"echo '' >> {output};"
        r"echo DEICODE Biplot QZV: [{input.visbiplotemp}]\(../{input.visbiplotemp}\){{target=\"_blank\"}} >> {output};"
        r"echo '' >> {output};"
        r"echo '### Beta Group Significance' >> {output};"
        r"echo '' >> {output};"
        r"echo Bray-Curtis QZV: [{input.visbcgs}]\(../{input.visbcgs}\){{target=\"_blank\"}} >> {output};"
        r"echo '' >> {output};"
        r"echo Jaccard QZV: [{input.visjacgs}]\(../{input.visjacgs}\){{target=\"_blank\"}} >> {output};"
        r"echo '' >> {output};"
        r"echo Weighted UniFrac QZV: [{input.viswugs}]\(../{input.viswugs}\){{target=\"_blank\"}} >> {output};"
        r"echo '' >> {output};"
        r"echo Unweighted UniFrac QZV: [{input.visuwugs}]\(../{input.visuwugs}\){{target=\"_blank\"}} >> {output};"
        r"echo '' >> {output};"
        r"echo '---' >> {output};"
        r"echo '' >> {output};"
        r"echo '## Tourmaline Config File' >> {output};"
        r"echo '' >> {output};"
        r"echo YAML: [{input.configfile}]\(../{input.configfile}\){{target=\"_blank\"}} >> {output};"
        r"echo '' >> {output};"
        r"echo '```' >> {output};"
        r"cat {input.configfile} >> {output};"
        r"echo '```' >> {output};"
