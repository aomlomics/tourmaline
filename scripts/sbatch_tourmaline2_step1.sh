...
#SBATCH -n 36
#SBATCH -t 48:00:00
#SBATCH -J tourmaline_array_qaqc
#SBATCH --array=
#SBATCH -o /dev/null
...
EXPECTED_OUTPUT="${THIS_SAMPLE}/${RUN_NAME}_Output/${RUN_NAME}-qaqc"

# Set the slurmout file name based on the run name
SLURMOUT_FILE="${RUN_NAME}_tourmaline2_qaqc_${SLURM_JOB_ID}_slurmout.out"
...
if [ -d "$EXPECTED_OUTPUT" ]; then
    ...
    # ======= PIPELINE EXECUTION =======
    echo "[$(date)] Starting QAQC for: $SAMPLE_NAME"

    # Load Conda environment manager
    source /PATH/TO/conda.sh

    # Activate Tourmaline environment
    conda activate tourmaline2

    # Change to Tourmaline GitHub directory
    cd PATH/TO/00_TOURMALINE_GITHUB

    # Run Tourmaline pipeline in QAQC mode
    ./tourmaline.sh --step qaqc --configfile "$CONFIG_FILE" --cores $SLURM_NTASKS
    EXIT_STATUS=$?
    ...
    if [ $EXIT_STATUS -eq 0 ]; then
        echo "[$(date)] Finished QAQC for: $SAMPLE_NAME successfully."
    else
