...
trim_params: "--discard-untrimmed -m 50"
sample_mapping_file: # optional if not using file names as samples
...
# Checks needed?
# 1. Check if fastq files exists
# 2. Check marker file exists
# 3. Opt. Check if manifest file exists


# Read the .tsv file
df = pd.read_csv(config["marker_info"], sep='\t', index_col=0)
# Convert the DataFrame to a dictionary
marker_dict = df.to_dict(orient='index')
# >>> dpath.get(x, '/a/b/43')
...
rule cutadapt_pe:
    input:
        ["config["fastq_path"]/{sample}.1.fastq", "config["fastq_path"]/{sample}.2.fastq"],
    output:
    ...
    run:
        ...
        -a "${primerF};required...${revcomp_primerR};optional" \
