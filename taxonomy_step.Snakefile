...
rule run_taxonomy:
    ...
    input:
        ...
        output_dir+config["run_name"]+"-taxonomy/"+config["run_name"]+"-taxa_sample_table_"+"l"+str(config["collapse_taxalevel"])+".tsv",
...
rule export_taxa_biom:
    ...
    output:
        taxa_table=output_dir+config["run_name"]+"-taxonomy/"+config["run_name"]+"-taxa_sample_table_"+"l"+str(config["collapse_taxalevel"])+".tsv",
    params:
    ...
    shell:
        ...
        "-o TEMP.tsv "
        "--to-tsv "
        "&& cat TEMP.tsv | tail -n +2 | sed 's/^#OTU ID/Taxon/' > {output.taxa_table} "
