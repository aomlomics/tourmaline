...
rule make_manifest_pe:
    ...
    shell:
        """
        echo "sample-id,forward-absolute-filepath,reverse-absolute-filepath" > {output}
        for f in {input.fwdreads}
        do
            echo "$(basename $f | sed 's/.1.fastq//'),$(pwd)/$f,$(pwd)/$(basename $f | sed 's/.1.fastq/.2.fastq/g')" >> {output}
        done
